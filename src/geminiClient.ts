...
/**
 * Validates the Gemini API key in the browser.
 */
...
/**
 * Generates chat responses directly in the browser.
 */
export const getGeminiChatResponse = async (
  ...
  return response.text || "죄송합니다. 답변을 생성하지 못했습니다. 다시 한 번 시도해 주세요.";
};

/**
 * Generates structured 4-Axis analysis data directly in the browser.
 */
export const getGeminiAnalysis = async (apiKey: string, text: string): Promise<AnalysisData> => {
  const ai = getAIClient(apiKey);
...
`;
  ...
  try {
    ...
    console.error("Failed to parse Gemini json output:", jsonText, err);
    throw new Error("분석 데이터를 정상적으로 파싱할 수 없습니다.");
  }
};
